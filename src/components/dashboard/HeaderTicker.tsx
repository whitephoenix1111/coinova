  ...
  return (
    <header style={{ height: "52px", background: "var(--bg-panel)", borderBottom: "1px solid var(--border-subtle)", display: "flex", alignItems: "center", padding: "0 16px", gap: "16px", flexShrink: 0 }}>
      <div className="flex items-center gap-3">
        <div className="pulse-dot" style={{ width: "6px", height: "6px", borderRadius: "50%", background: isConnected ? "var(--green)" : "var(--text-tertiary)", boxShadow: isConnected ? "0 0 6px var(--green)" : "none", flexShrink: 0 }} />
      ...
      <div className="flex items-baseline gap-2">
        <span style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "20px", color: isPositive ? "var(--green)" : "var(--red)", textShadow: isPositive ? "0 0 12px rgba(0,230,118,0.3)" : "0 0 12px rgba(255,61,90,0.3)" }}>
          {ticker ? fmt(ticker.price) : "—"}
        </span>
        <span style={{ fontFamily: "var(--font-display)", fontSize: "12px", fontWeight: 500, color: isPositive ? "var(--green)" : "var(--red)", background: isPositive ? "var(--green-bg)" : "var(--red-bg)", padding: "2px 6px", borderRadius: "3px" }}>
